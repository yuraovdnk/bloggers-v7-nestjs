...
export class CommentsRepository {
  ...
  async setLikeStatus(commentId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId, likeStatus: string) {
  ...
  async setNewLike(commentId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId, likeStatus: string) {
    const res = await this.commentModel.updateOne(
      { _id: commentId },
      {
  ...
  async unsetLike(commentId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId) {
    const res = await this.commentModel.updateOne({ _id: commentId }, { $pull: { 'likesInfo.likes': { userId } } });
    return res.matchedCount;
  }
