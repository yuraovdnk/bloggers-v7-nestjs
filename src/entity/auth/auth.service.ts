...
export class AuthService {
  ...
  async signUp(registrationDto: RegistrationDto) {
    const existUser = await this.usersRepository.findByLoginOrEmail(registrationDto.login, registrationDto.email);
    if (existUser) {
  ...
  async checkCredentials(loginDto: LoginDto) {
    ...
    const isValidPassword = await bcrypt.compare(loginDto.password, candidate.accountData.passwordHash);
    if (!isValidPassword) {
  ...
  async generateTokens(userId: mongoose.Types.ObjectId) {
    ...
    const accessToken = this.jwtService.sign(payload, { secret: constans.secretAccessToken, expiresIn: '1d' });
    const refreshToken = this.jwtService.sign(payload, { secret: constans.secretRefreshToken, expiresIn: '2d' });
