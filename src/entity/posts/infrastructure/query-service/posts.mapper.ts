...
export class PostsMapper {
  static mapPagingPosts(pagingPosts: PaginatedItems<AggregatedPostType>, userId: mongoose.Types.ObjectId) {
  ...
  private static mapLikes(likesInfo, userId: mongoose.Types.ObjectId) {
    const myStatus = likesInfo.likes.find((item) => userId && item.userId._id.toString() === userId.toString());
    return {
