...
export class UsersRepository {
  ...
  async getAllUsers(queryParams: QueryParamsType) {
    ...
    const items = await this.userModel.find({ filter }).skip(queryParams.skip).limit(queryParams.pageSize);
    return paginator(items, queryParams);
  }
  ...
  async updateConfirm(id: mongoose.Types.ObjectId): Promise<boolean> {
    const res = await this.userModel.updateOne({ _id: id }, { 'emailConfirmation.isConfirmed': true });
    return res.acknowledged;
  }
  async updateCode(id: mongoose.Types.ObjectId, code: string): Promise<boolean> {
    const res = await this.userModel.updateOne({ id }, { $set: { 'emailConfirmation.confirmationCode': code } });
    return res.acknowledged;
  }
}
