...
export class UsersService {
  ...
  async getUsers(queryParams: QueryParamsType) {
    return this.usersRepository.getAllUsers(queryParams);
  }

  async addUser(createUserDto: RegistrationDto) {
    const existUser = await this.usersRepository.findByLoginOrEmail(createUserDto.login, createUserDto.email);
    if (existUser) {
